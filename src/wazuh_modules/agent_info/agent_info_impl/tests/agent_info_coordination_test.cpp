...
// Regression test for issue #36358: when FIM reports first_sync_completed=false on
// is_pause_completed, agent-info must extend the pause poll budget beyond the steady-state
// 30 attempts. The first post-upgrade FIM sync on macOS can take several minutes; without
...
// Regression guard for issue #36358: when FIM reports first_sync_completed=true (steady state),
// the pause poll budget must stay at 30 attempts. We must not silently widen the window for the
// common case.
...
// Issue #36358 (Option C): FIM must be paused before SCA/syscollector so that those modules
