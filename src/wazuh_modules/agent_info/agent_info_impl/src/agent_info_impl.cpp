...
// FIM is paused first because its pause is asynchronous and may need to wait for a long
// first-sync iteration to complete. Pausing FIM ahead of SCA/syscollector avoids leaving
// those modules idle during the (potentially multi-minute) FIM pause poll window.
...
bool AgentInfoImpl::pollFimPauseCompletion(const std::string& moduleName)
{
    ...
    // from scratch and on macOS can hold fim_scan_mutex for several minutes. The 30 s
    ...
    for (int attempt = 1; attempt <= maxAttempts; ++attempt)
    {
        ...
        try
        {
            ...
            if (pollJson.contains("data") && pollJson["data"].contains("status"))
            {
                ...
                // The field is optional (only emitted by 5.x+ FIM); absent means treat as completed.
